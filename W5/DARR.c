...
int init_array(dyn_array *the_array, int size)
{
    ...
    the_array->all_reviews = (review *)calloc(sizeof(review), size);
...
int add_review(dyn_array *the_array)
{
    ...
    name[strlen(name) - 1] = '\0';  // Replace newline

    printf("* Enter a score for the restaurant: ");
    scanf("%d", &score);
    ...
    // WE DIDN'T GET TO THIS IN TUTORIAL, SO NOW IT'S YOUR JOB TO CODE IT!
